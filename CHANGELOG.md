...
- Complete web login when Authress redirects with `nonce` + `iss` and stores tokens in cookies (custom-domain cookie response location)
- Fall back to `PATCH /api/session` with credentials when Authress cookies are not readable from JavaScript
- Redirect authenticated users away from `/auth/callback` to `/home`
